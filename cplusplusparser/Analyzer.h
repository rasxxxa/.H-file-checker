...
private:
	...
	std::unordered_map<std::string, int> GetMethodOcurenceWithoutGlobalPointer(const MethodsVariableCounter& parser, const std::unordered_map<std::string, std::vector<std::string>>& files, const std::string& classForParser);
	...
	std::unordered_map<std::string, std::vector <std::string>> m_allFilesCPP;
	std::unordered_map<std::string, std::vector <std::string>> m_allFilesH;
